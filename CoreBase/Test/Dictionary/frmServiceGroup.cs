...
{
    public partial class frmServiceGroup : CoreBase.WinForm.Dictionary.Dictionary
    {
        const string BANNER_CMDKEY = "ServiceGroup";
        const string BANNER_ADD_CMDKEY = "ServiceGroup_add";
        const string BANNER_DEL_CMDKEY = "ServiceGroup_del";
        const string BANNER_EDIT_CMDKEY = "ServiceGroup_edit";
        const string BANNER_LIST_CMDKEY = "ServiceGroup_list";
        DataRow _dr;
        DataTable _ServiceGroup;
        string _tableName = "zServiceGroup";
        string _Mode = "";
        string _idName = "ServiceGroupID";
        int _postion = 0;
        ...
        protected override void FillData()
        {
            ...
            CreateBinding(txtServiceGroupName);
            CreateBinding(txtGroupStt);
            ...
            try
            {
                ...
                if (changedRows != null)
                {
                    foreach (DataRow dr in changedRows.Rows)
                    {
                        ...
                        if (!isSuccess)
                        {
                            listError += "Save error Service Group: " + dr["ServiceGroupName"].ToString() + ". \n";
                        }
                    }
                }
        ...
        private void LoadData()
        {
            using (DictionaryDAL dal = new DictionaryDAL(_tableName))
            {
                if (NailApp.IsAdmin())
                {
                    Bds.DataSource = _ServiceGroup = dal.GetData();
                }
                else
                {
                    _ServiceGroup = dal.GetData().Select("branchId = " + NailApp.BranchID, "").CopyToDataTable();
                    Bds.DataSource = _ServiceGroup;
                }
            }
            LoadGrid();
            _postion = 0;
        ...
        {
            _ServiceGroup.DefaultView.Sort = "ServiceGroupName";
            GridDetail.DataSource = _ServiceGroup;
            ...
            GridDetail.Columns["ServiceGroupID"].Visible = false;
            //GridDetail.Columns["branchId"].HeaderText = "Branch";
            GridDetail.Columns["ServiceGroupName"].HeaderText = "Name";
            GridDetail.Columns["GroupStt"].HeaderText = "Number Order";
        ...
        private void GridDetail_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex >= 0)
                {
                    ...
                    txtServiceGroupName.Text = row.Cells["ServiceGroupName"].Value.ToString();
                    txtGroupStt.Text = row.Cells["GroupStt"].Value.ToString();
